...
# calculate average outcomes in 12 months before training
did$mean <-
  pj_balanced %>%
...
pj_balanced %>%
  mutate(relative = period - first_trained) %>%
  filter(between(relative, -12, -1)) %>%
  summarize_at(vars(complaints, sustained, force),
               ~ sum(.) / 12) %>%
  pivot_longer(cols = everything()) %>%
  pluck("value")
...
average_att <-
  ...
  left_join(transmute(did, outcome, mean = round(mean, 3))) %>%
  mutate(
...
average_att %>%
  ...
  ggsave("output/figures/average_effects.pdf",
         height = 5, width = 9)
...
did_figures <-
  transmute(
    ...
    cal     = list(
      plot_cs(calendar, mean = mean, ylab = NULL) +
        scale_x_continuous(NULL)
    ),
    cal_zoom = list(
      plot_cs(calendar,
              ylab = glue("Calendar ATT: ",
                          str_to_title(word(outcome, 2, sep = "_")),
                          " per officer"),
              time = c(19, 60), mean = mean)
    ),
    sel      = list(
      plot_cs(selective,
              ylab = glue("Cohort ATT: ",
                          str_to_title(word(outcome, 2, sep = "_")),
                          " per officer"),
              mean = mean)
    )
...
# write dynamic effects
wrap_plots(did_figures$dyn, ncol = 3) +
  ggsave("output/figures/dynamic_effects.pdf",
         height = 4, width = 10)

# write calendar effects
wrap_plots(c(did_figures$cal, did_figures$cal_zoom),
           ncol = 3, heights = c(0.25, 0.75)) +
  ggsave("output/figures/calendar_effects.pdf",
         height = 5, width = 10)

# write selective effects
wrap_plots(did_figures$sel, ncol = 3) +
  ggsave("output/figures/selective_effects.pdf",
         height = 4, width = 10)
