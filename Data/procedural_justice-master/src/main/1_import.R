...
officers <-
  here("Data/procedural_justice-master/data/officers/officers.csv") %>%
  read_csv() %>%
  transmute(
...
accused <-
  bind_rows(
    here("Data/procedural_justice-master/data/complaints/accused_2000_2016.csv") %>%
      read_csv() %>%
      transmute(uid = UID, cr_id, finding = final_finding),
    here("Data/procedural_justice-master/data/complaints/accused_2000_2018.csv") %>%
      read_csv() %>%
      transmute(uid = UID, cr_id, finding = final_finding)
  ) %>%
  group_by(uid, cr_id) %>%
...
complaints <-
  bind_rows(
    here("Data/procedural_justice-master/data/complaints/complaints_2000_2016.csv") %>%
      read_csv() %>%
      transmute(cr_id, date = incident_date),
    here("Data/procedural_justice-master/data/complaints/complaints_2000_2018.csv") %>%
      read_csv() %>%
...
settlements <-
  here("Data/procedural_justice-master/data/settlements/settlements_1952_2016.csv") %>%
  read_csv() %>%
...
type <-
  here("Data/procedural_justice-master/data/force/trr_actions_responses_2004_2016.csv") %>%
  read_csv() %>%
...
trr <-
  here("Data/procedural_justice-master/data/force/trr_main_2004_2016.csv") %>%
  read_csv() %>%
  transmute(trr_id, date = trr_date)

user <-
  here("Data/procedural_justice-master/data/force/trr_officers_2004_2016.csv") %>%
  read_csv() %>%
...
training <-
  here("Data/procedural_justice-master/data/training/training.csv") %>%
  read_csv() %>%
...
save.image(here("Data/procedural_justice-master/products/rdata/1_import.RData"))
